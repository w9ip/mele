...
def post_list(request, tag_slug=None):
    ...
    try:
        ...
        # Если page_numver не целое число, то выдать первую страницу.
        posts = paginator.page(1)
    except EmptyPage:
        # Если page_number вне диапазона, выдать последнюю страницу.
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = f"{cd['name']} рекоммендует вам прочитать {post.title}"
            message = (f"Прочитайте {post.title} в {post_url}\n\n"
                       f"комментарии {cd['name']}: {cd['comments']}")
